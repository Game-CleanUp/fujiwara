...
CCollisionManager *CCollisionManager::spInstance = 0;

CCollisionManager* CCollisionManager::Get()
{
	if (spInstance == 0)
	{
		spInstance = new CCollisionManager();
	}
	return spInstance;
}
...
{
	int sPri = collider->mPriority + RANGE; //開始位置（大）
	int ePri = collider->mPriority - RANGE; //終了位置（小）
	//現在位置を先頭にする
	CCollider *pos = (CCollider*)mpHead->mpNext;
	//開始位置まできたら終了する
